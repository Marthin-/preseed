...
if [ $# -lt 1 ]; then
    ...
    OPTIONS="--virt-type=kvm --name=$hostname --vcpus=$cpu --memory=$memory --os-variant=linux --disk pool=$pool,size=$disk_size,format=$disk_format --network bridge=$net_bridge --console pty,target_type=serial --noautoconsole --location=$location --initrd-inject=$preseed_file -x 'auto=false,text,console=ttyS0,115200n8,serial'"
